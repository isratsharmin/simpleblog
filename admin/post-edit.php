...
if(isset($_POST['form1'])) {
	...
	try {
		...
		if(empty($_FILES["post_image"]["name"])) {
			...
			$statement = $db->prepare("UPDATE post SET post_title=?, post_description=?, cart_id=?, tag_id=? WHERE post_id=?");
			$statement->execute(array($_POST['post_title'],$_POST['post_description'],$_POST['cart_id'],$tag_ids,$id));
			...
			$statement = $db->prepare("UPDATE post SET post_title=?, post_description=?,post_image=?, cart_id=?, tag_id=? WHERE post_id=?");
			$statement->execute(array($_POST['post_title'],$_POST['post_description'],$f1,$_POST['cart_id'],$tag_ids,$id));
...
foreach($result as $row)
{
	...
	{
		?><option value="<?php echo $row['cart_id']; ?>" selected><?php echo $row['cart_name']; ?></option><?php
	}
	else
	{
		?><option value="<?php echo $row['cart_id']; ?>"><?php echo $row['cart_name']; ?></option><?php
	}
