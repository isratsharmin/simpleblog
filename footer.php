			...
			<div class="list">
				...
				<ul>
					<?php 
						...
						{
							?>
							<li><a href="category.php?id=<?php echo $row['cart_id']; ?>"><?php echo $row['cart_name']; ?></a></li>
							<?php
						}
					?>
				...
				<?php 
					$j=0;
					$statement = $db->prepare("SELECT distinct(post_date) FROM post ORDER BY post_date DESC");
					$statement->execute();
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);
					foreach($result as $row)
					{
