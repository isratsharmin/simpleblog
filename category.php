...
include "config.php";
			...
			$statement = $db->prepare("SELECT * FROM post WHERE cart_id=? ORDER BY post_id DESC");
			...
			$statement = $db->prepare("SELECT * FROM post WHERE cart_id=? ORDER BY post_id DESC LIMIT $start, $limit");
			$statement->execute(array($id));
